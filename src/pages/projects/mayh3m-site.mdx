...
Prisma for the data access, PlanetScale(MySql) for database and is hosted on Vercel.
...
I built{" "}<a href="http://www.mayh3m.xyz/">Mayh3m.xyz</a> {" "} to allow users to apply for the
...
out the application form. The application form is built with React Hook Form and the data is sent to the
server using trpc. The server then validates the data and saves it to the database. Once a user has applied for
a drop, they receive a confirmation email and can then not apply for that drop again.
The admins can then review the applications and accept or reject them. The admins can also view all the
applications and filter them by status and drop.
